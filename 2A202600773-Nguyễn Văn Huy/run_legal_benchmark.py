...
LAW_CSV_PATH = BASE_DIR / "data" / "selected_legal_domain_laws.csv"
ARTICLE_CSV_PATH = BASE_DIR / "data" / "selected_legal_articles_by_article.csv"
OUTPUT_PATH = BASE_DIR / "report" / "legal_benchmark_results.md"
...
def load_documents(article_csv_path: Path, law_csv_path: Path) -> list[Document]:
    ...
    with article_csv_path.open("r", encoding="utf-8-sig", newline="") as file:
        ...
        for row in reader:
            ...
            documents.append(Document(id=f"{doc_id}:{article_index}", content=content, metadata=metadata))
    return documents
...
def write_report(results: list[dict], output_path: Path) -> None:
    ...
    lines = [
        ...
        "Join key: `doc_id`.",
        "",
        "Strategy: `chunk_by_article` + topic filter + legal rerank.",
        "",
        f"Top-3 hit rate without filter: {unfiltered_hits}/5.",
        f"Top-3 hit rate with topic filter + legal rerank: {filtered_hits}/5.",
        "",
    ]
...
def print_console_summary(results: list[dict]) -> None:
    ...
    for index, item in enumerate(results, start=1):
        benchmark = item["benchmark"]
        unfiltered_hit = top3_hit(item["unfiltered"], benchmark)
        filtered_hit = top3_hit(item["filtered"], benchmark)
        unfiltered_hits += int(unfiltered_hit)
        filtered_hits += int(filtered_hit)
        top = item["filtered"][0]
        metadata = top["metadata"]
        print(
            f"{index}. filtered_hit={filtered_hit} | "
            f"top1={metadata['short_title']} {metadata['article_no']} - {metadata['article_title']} | "
            f"relevant={is_relevant(top, benchmark)} | score={top['score']:.4f}"
        )
    print(f"\nTop-3 hit rate without filter: {unfiltered_hits}/5")
    print(f"Top-3 hit rate with topic filter + legal rerank: {filtered_hits}/5")
...
def main() -> None:
    ...
    print(f"\nWrote detailed benchmark results to {OUTPUT_PATH}")
