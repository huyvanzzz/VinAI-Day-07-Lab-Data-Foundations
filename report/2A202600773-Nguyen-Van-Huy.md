...
|---|---|---:|---|---|
| Tôi - Nguyễn Văn Huy | `chunk_by_article` + `all-MiniLM-L6-v2` + topic filter + legal rerank | 10/10 | Dùng embedding local thật hơn mock, giữ đúng đơn vị Điều luật, top-1 đúng cả 5 query sau filter/rerank | Chạy chậm hơn mock và vẫn cần metadata/rerank tốt |
| Dương Quang Minh | Custom `chunk_by_article` trên bộ `selected_legal.md` rút gọn + topic filter | 10/10 | Strategy đơn giản, bám đúng cấu trúc Điều luật, dễ giải thích và dễ trace gold answer | Dữ liệu benchmark là Markdown rút gọn, chưa dùng toàn bộ 1,897 article chunks |
| Trần Quốc Khánh | `RecursiveChunker` trên bộ tài liệu sample/educational KB | khoảng 7/10 strategy, 3/5 query relevant trong top-3 | Phù hợp tài liệu Markdown/text hỗn hợp, giữ paragraph/heading tốt hơn fixed-size | Không cùng legal corpus chính; mock embedding làm retrieval chưa ổn định |
| Trần Trung Kiên | `RecursiveChunker` tối ưu theo cấu trúc Bộ luật Hình sự | 10/10 trên benchmark BLHS riêng | Tận dụng ranh giới Điều/Khoản/Câu, tốt khi tập trung vào một văn bản luật lớn | Benchmark hẹp hơn, chủ yếu xoay quanh Bộ luật Hình sự |
| Lã Duy Anh | `ArticleBasedLegalChunker` + metadata filter + lexical rerank | 10/10 | Dùng full legal corpus 17 luật/1,897 chunks, so sánh rõ article-level chunking với baseline | Rerank phụ thuộc rule/hints, cần kiểm thử thêm với query production không có expected article |
| Nguyễn Anh Kiệt | `chunk_by_article` + topic filter + legal rerank | 10/10 | Pipeline gọn, tận dụng metadata `topic`, `article_no`, `article_title`; top-1 đúng 5/5 | Vector score chỉ là tín hiệu ban đầu, kết quả phụ thuộc nhiều vào metadata và rerank |

**Strategy nào tốt nhất cho domain này? Tại sao?**  
Với domain pháp luật, nhóm strategy tốt nhất là **article-level chunking (`chunk_by_article` / `ArticleBasedLegalChunker`) kết hợp metadata filter và rerank theo cấu trúc pháp lý**. Các report của tôi, Lã Duy Anh và Nguyễn Anh Kiệt đều cho thấy khi dùng toàn bộ hoặc gần toàn bộ legal corpus, việc giữ nguyên từng Điều luật giúp truy vết nguồn rõ hơn và giảm nguy cơ cắt mất căn cứ pháp lý. Topic filter giúp đưa search về đúng mảng luật, còn rerank theo `article_title`, `article_no` hoặc keyword overlap giúp chọn đúng Điều khi nhiều chunk có từ khóa giống nhau.

So với nhóm article-level, `RecursiveChunker` của Trần Trung Kiên vẫn hiệu quả khi benchmark hẹp trong một văn bản như Bộ luật Hình sự, vì separator có thể bám theo ranh giới Điều/Khoản. Tuy nhiên nếu mở rộng ra nhiều luật, metadata filter vẫn rất cần thiết. Report của Trần Quốc Khánh cho thấy với corpus không phải pháp luật hoặc dùng mock embedding, recursive chunking giữ context tốt nhưng retrieval quality có thể thấp hơn nếu không có filter/rerank đủ mạnh. Vì vậy, cho legal benchmark chung của nhóm, lựa chọn hợp lý nhất là `chunk_by_article` + `topic` filter + legal rerank.
